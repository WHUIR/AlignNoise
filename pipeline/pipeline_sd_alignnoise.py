...
from utils.attn_utils import fn_show_attention, fn_smoothing_func, fn_get_topk, fn_clean_mask, fn_get_otsu_mask,fn_show_mask_attention
from tqdm import tqdm
import matplotlib.pyplot as plt

from utils.fi_utils import (
    get_freq_filter,
    freq_mix_2D,
)
...
class StableDiffusionInpaintPipeline(
    ...
    @torch.no_grad()
    def init_filter(self, height, width, filter_params):
        batch_size = 1
    ...
    def fn_compute_loss_mask(
        ...
        if len(self_attention_map_per_token_list) > 0:
            self_attention_maps_stacked = torch.stack(self_attention_map_per_token_list, dim=0)
            
            combined_attention = torch.sum(self_attention_maps_stacked, dim=0)
            ...
            mask_score = torch.mean(combined_attention[mask_resized > 0])
            non_mask_score = torch.mean(combined_attention[mask_resized == 0])
            
            AS_src = non_mask_score
            AS_out = mask_score
            AAM_loss = AS_out / (AS_src + 1e-6) + 1 * torch.max(torch.tensor(0.0, device=self._execution_device), torch.tensor(0.15, device=self._execution_device) - AS_out)  # ASout/(ASin1+ε) + λ·max(0,τ-ASin1)
        else:
            AS_src = torch.tensor(0.0, device=self._execution_device, requires_grad=True)
            AS_out = torch.tensor(0.0, device=self._execution_device, requires_grad=True) 
            AAM_loss = torch.tensor(0.0, device=self._execution_device, requires_grad=True)
    ...
    def fn_AAM(
        ...
        masked_image_latents = masked_image_latents.clone().detach()
        ...
        for iteration in tqdm(range(max_step)):
            ...
            mask_first = mask[0:1]
            masked_image_latents_first = masked_image_latents[0:1]
            optimized_latents = torch.cat([optimized_latents, mask_first, masked_image_latents_first], dim=1)
            ...
            AAM_loss_list, AS_src_list, AS_out_list = [], [], []

            for i, t in enumerate(timesteps):
                if i >= denoising_step_for_loss: break

                if use_grad_checkpoint:#  
                    noise_pred_text = checkpoint.checkpoint(self.unet, optimized_latents, t, text_embeddings[0].unsqueeze(0), use_reentrant=False).sample
                else: noise_pred_text = self.unet(optimized_latents, t, encoder_hidden_states=text_embeddings[0].unsqueeze(0)).sample
                AAM_loss, AS_src, AS_out = self.fn_compute_loss_mask(K=1, mask=mask_first)
                
                AAM_loss_list.append(AAM_loss), AS_src_list.append(AS_src), AS_out_list.append(AS_out)
                
            AAM_loss      = sum(AAM_loss_list) / denoising_step_for_loss
            AS_src = max(AS_src_list)
            AS_out  = max(AS_out_list)
            
            AAM_loss_list         = [_.item() for _ in AAM_loss_list]
            AS_src_list    = [_.item() for _ in AS_src_list]
            AS_out_list     = [_.item() for _ in AS_out_list]
        ...
        if AS_out <= 1e-6: AS_out = AS_out + 1.
        return optimized_latents, optimization_succeed, AS_src + AS_out 
        ...
        for iter in range(num_iters):
            if iter == 0:
                with torch.enable_grad():
                    optimized_latents, optimization_succeed, cross_AS_out = self.fn_AAM(
                        latents=latents,
                ...
                diffuse_timesteps = torch.full((batch_size,), int(current_diffuse_timestep), device=device).long()
                ...
                if self.do_classifier_free_guidance:
                    ...
                    init_mask = mask
                latents = z_T * (1 - init_mask) + latents * init_mask
